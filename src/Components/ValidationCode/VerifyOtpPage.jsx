...
const VerifyOtpPage = () => {
  ...
  useEffect(() => {
    ...
    if (accessCode) setOtp(accessCode);
  }, [user, navigate, accessCode]);
  ...
  const handleVerify = (e) => {
    e.preventDefault();
    if (!otp || otp.length < 5) return; // basic validation
    dispatch(verifyOtp({ otp, userId: user?.id || user?.userId }))
      .unwrap()
  ...
  const handleResend = () => {
    if ((user?.id || user?.userId) && timer === 0) {
      dispatch(resendOtp(user.id || user.userId));
      setTimer(30);
    }
  };
